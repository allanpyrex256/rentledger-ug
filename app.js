(function () {
  ...
  async function createAccount(event) {
    ...
    if (!selectedPlanOption) {
      showToast("Choose Starter or Professional before starting the 30-day free trial.");
      return;
    }
    if (!paymentMethod) {
    ...
    if (!ui.accountBillingConsent.checked) {
      showToast("Accept the terms and privacy policy to start the 30-day free trial.");
      return;
    }
    ...
    try {
      ...
      setView("properties");
      showToast(`${selectedPlanOption.plan} 30-day free trial opened.`);
    } catch (error) {
  ...
  function updateSignupBillingSummary() {
    ...
    if (!planOption) {
      ui.accountTrialSummary.textContent =
        "Select Starter or Professional, choose a payment method, and confirm your details before starting the 30-day free trial. No charges are made today.";
      return;
    }
    if (!paymentMethod) {
      ui.accountTrialSummary.textContent =
        `${planOption.plan} selected. Choose a payment method to confirm how subscription billing works after the 30-day trial. No charges are made today.`;
      return;
    }
    ...
    ui.accountTrialSummary.textContent =
      `${planOption.plan} starts with a 30-day free trial. No charges are made today. After ${formatDate(nextBillingDate)}, the subscription is ${formatMoney(planOption.fee)}/month by ${signupPaymentMethodLabel(paymentMethod)}${contactLabel} unless you cancel before renewal.`;
  }
